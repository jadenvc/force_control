"""Pyrite-style episode recording for the insertion teleop task.

Deliberately a sibling of ``pyrite_recorder.py`` (and structurally identical
to ``sanding_recorder.py``), not an edit to either in place --
``PyriteEpisodeRecorder.record_sample``/``commit`` hardcode FlipUp-specific
fields directly in their bodies, and ``SandingEpisodeRecorder`` hardcodes
sanding's dose-grid/coverage fields, neither of which apply here. This
module reuses the parts of ``pyrite_recorder.py`` that ARE generic -- the
growable per-key sample buffer and the JSON-metadata helper -- and
reimplements ``record_sample``/``commit`` against ``InsertionEnv`` instead.

On-disk layout (same shape as pyrite_recorder.py's/sanding_recorder.py's,
different schema name so insertion datasets never collide with either on
disk):

    data/episode_N/{rgb_0, ts_pose_fb_0, ts_pose_command_0, wrench_0,
                    wrench_ground_truth_0, wrench_filtered_0, contact_force_n,
                    peg_tip_depth_m, broken, success, ...}
    meta/{episode_rgb0_len, episode_robot0_len, episode_wrench0_len}

Unlike sanding (whose per-cell dose grid is deliberately NOT recorded
per-step because of its size), insertion has no large per-step grid to
worry about -- everything task-specific here is a handful of scalars.
"""
...
SCHEMA_NAME = "pyrite_insertion_sim"
SCHEMA_VERSION = 1
DEFAULT_SAMPLE_HZ = 1000.0


class InsertionEpisodeRecorder:
    """Accumulate one insertion episode in memory and atomically append it to Zarr."""
    ...
    def record_sample(
        ...
        *,
        ...
        wrench_exact, _ = env.peg_contact_force()
        wrench = np.concatenate([wrench_exact, np.zeros(3)])  # no torque sensor on the exact path
        wrench_filtered = env.wrist_wrench_filtered()
        ...
        # Filtered simulated wrist F/T sensor reading (see
        # insertion_teleop.py's FTSensorFilter/DynamicFilter) -- the piece
        # sanding/flipup's recorders don't have an equivalent of, since
        # those tasks only ever recorded the exact solver contact force.
        self._append("wrench_filtered_0", wrench_filtered)
        self._append("normal_force_n", env.normal_force_n())
        self._append("peg_tip_depth_m", env.peg_tip_depth_m())
            ...
            episode.attrs.update(
                {
                    ...
                    "final_task_metric_value": float(
                        0.0 if final_task_metric_value is None else final_task_metric_value
                    ),
                    "sample_count": count,
